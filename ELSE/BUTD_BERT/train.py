"""Training script"""
import sys
sys.path.append("/home/fu/PycharmProjects/ESA")
import os
import time
import numpy as np
import torch
...
def main():
    ...
    tokenizer = BertTokenizer.from_pretrained('/home/fu/fu-tools/bert-base-uncased')
    ...
    for epoch in range(start_epoch, opt.num_epochs):
        ...
        if opt.data_name == 'f30k':
            test_loader = image_caption.get_test_loader('test', opt.data_name, tokenizer,
                                                opt.batch_size, opt.workers, opt)
        elif opt.data_name == 'coco':#COCO全部
            test_loader = image_caption.get_test_loader('testall', opt.data_name, tokenizer,
                                                opt.batch_size, opt.workers, opt)    
        logger.info('Computing results...')
        rsum_test = validate(opt, test_loader, model)
        '''
